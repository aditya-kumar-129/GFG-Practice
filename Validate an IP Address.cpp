...
public:
  int isValid(string s) {
    ...
    string str = "";
    // count the no of . present in the string 
    for (int i = 0; i < n; i++) {
      if (s[i] == '.') c1++;
    }
    //if it is less than three then return 0
    if (c1 != 3) return 0;
    for (int i = 0; i < n; i++) {
      ...
      // if any alphabetical character is present then return 0
      ...
      00.00.00.00
      */
      if (str[0] == '0' && str.length() > 1) return 0;
    }
    return 1;
  }
};