...
int findOnce(int arr[], int n) {
  ...
  while (low <= high) {
    int mid = (low + high) / 2;
    if (arr[mid] != arr[mid - 1] && arr[mid] != arr[mid + 1]) return arr[mid];
    else if ((arr[mid] == arr[mid - 1] && mid % 2 != 0) || (arr[mid] == arr[mid + 1] && mid % 2 == 0)) low = mid + 1;
    else high = mid - 1;
  }
}
...
int findOnce(int arr[], int n) {
  ...
  while (i <= j) {
    ...
    else if (arr[j] != arr[j - 1]) return arr[j];
    i += 2;
    j -= 2;
  }
  return -1;
