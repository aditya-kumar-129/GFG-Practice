...
public:
  vector<int> singleNumber(vector<int> nums) {
    vector<int> ans;
    int num = 0;
    for (int it : nums) num ^= it;
    int rightmost_set_bit = num & ~(num - 1);
    int first_non_repeating_number = 0, second_non_repeating_number = 0;
    for (int it : nums) {
      if (rightmost_set_bit & it) first_non_repeating_number ^= it;
      else second_non_repeating_number ^= it;
    }
...
vector<int> singleNumber(vector<int> nums) {
  ...
  */
  ...
  int num1 = 0, num2 = 0;
  for (int i = 0;i < nums.size();i++) {
    if ((nums[i] & (1 << pos)) > 0) num1 = num1 ^ nums[i];
    else num2 = num2 ^ nums[i];
  }
  vector<int> ans;
  if (num1 > num2) ans.push_back(num2), ans.push_back(num1);
  else ans.push_back(num1), ans.push_back(num2);
  return ans;
}